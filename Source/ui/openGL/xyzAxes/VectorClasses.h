...
#ifndef _VectorClasses_H_
#define _VectorClasses_H_
...
inline float dotProduct(const Point3D& p, const Vector3D& v)
{
    return (p * v);
}
...
inline Vector3D crossProduct(const Point3D& p, const Vector3D& v)
{
    return (p % v);
}
...
{
    float magnitudeOfV1 = magnitude(v1);
    float magnitudeOfV2 = magnitude(v2);
    float dotProductOfV1AndV2 = dotProduct(v1, v2);
    float angleInRadiansBetweenV1AndV2 = acos(dotProductOfV1AndV2 / (magnitudeOfV1 * magnitudeOfV2));
    // If the dot product is negative, that the vectors aren't pointing in the same direction.
    // Add pi to the result...
    //if (dotProductOfV1AndV2 < 0)
    //    angleInRadiansBetweenV1AndV2 += 3.1415926f;
    return angleInRadiansBetweenV1AndV2;
}
